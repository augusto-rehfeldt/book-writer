...
def export_epub(
    ...
    provider_info = load_provider_info(project_path)
    ...
    author = str(project_data.get("init", {}).get("author_name") or "AI Book Creator")
    ...
    # Try the injected description, fallback to the saved one if it was already generated,
    # and ultimately fallback to the raw prompt if this is a manual export with missing data.
    final_description = description
    if not final_description:
        final_description = project_data.get("ebook", {}).get("description")
    if not final_description:
        final_description = project_data.get("init", {}).get("book_idea", "Generated by AI Book Creator.")
    ...
    with zipfile.ZipFile(output_file, "w") as epub:
        ...
        _write_frontmatter(epub, title, author, provider_info)
...
def load_provider_info(project_path: Path) -> dict[str, Any]:
    data = _read_json(project_path / "provider_state.json")
    info = data if isinstance(data, dict) else {}
    used = _read_json(project_path / "models_used.json")
    if isinstance(used, list):
        info["models_used"] = used
    return info
...
def text_ai_tools(provider_info: dict[str, Any]) -> list[str]:
    """Every model that wrote or revised the text, as 'Service (model)'.

    models_used.json is the record; without it (projects older than the
    record) fall back to the model remembered for the selected provider.
    """
    used = provider_info.get("models_used") or []
    if not used and provider_info.get("provider"):
        provider = str(provider_info["provider"])
        model = provider_info.get(f"{provider.replace('-', '_')}_model") or provider_info.get("writing_model")
        used = [[provider, model or "unknown model"]]
    return [f"{SERVICE_NAMES.get(service, service)} ({model})" for service, model in used]


def ai_credits(provider_info: dict[str, Any]) -> list[str]:
    credits = text_ai_tools(provider_info) or ["Unrecorded language model"]
    source = os.getenv("AI_BOOK_COVER_SOURCE", "pollinations").strip().lower()
    cover = {"perchance": "Perchance", "pollinations": "Pollinations"}.get(source)
    if cover:
        credits.append(f"{cover} (cover image)")
    return credits
...
def _resolve_book_title(project_data: dict[str, Any], project_path: Path, chapters: list[EbookChapter]) -> str:
    ...
    if str(init_data.get("book_title", "")).strip():
        return str(init_data["book_title"]).strip()[:120]
...
"""
    ...
    # 2. Disclaimer Page
    credits = "".join(f"<li>{html.escape(line)}</li>" for line in ai_credits(provider_info))
...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
  ...
  <body>
    <article class="chapter">
      <h1>Disclaimer</h1>
      <p>This book was entirely generated by Artificial Intelligence. All characters, locations, events, and scenarios described within are either products of the AI's generation or are used fictitiously. Any resemblance to actual persons, living or dead, or actual events is purely coincidental.</p>
      ...
      <p><strong>Models and Services Used:</strong></p>
      <ul>{credits}</ul>
    </article>
  </body>
</html>
